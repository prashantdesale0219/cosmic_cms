    ...
    <section className="relative bg-black h-[650px] sm:h-[750px] lg:h-screen overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit="exit"
        >
          <div className="absolute inset-0 bg-gradient-to-b from-black/80 to-[#142334]/80" />
        </motion.div>
      </AnimatePresence>
...
                <span
                ...
                >
                  {typeof s.icon === 'string' ? (
                    <span 
                      className={`h-12 w-12 ${
                        activeItem ? 'text-[#cae28e]' : 'text-white'
                      }`}
...
                    activeItem ? 'bg-[#cae28e]' : 'bg-white/40'
                  }`}
                />
                <span
                  className={`absolute -right-6 top-1/2 -translate-y-1/2 text-xl font-bold ${
                    activeItem ? 'text-[#cae28e] scale-125 transition-transform duration-300' : 'text-white/70'
                  }`}
                >
                  {s.num}
                </span>
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-center px-4 max-w-3xl w-full">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit="exit"
            >
              <p className="text-[#cae28e] tracking-wider text-sm md:text-lg mb-4 font-space-grotesk">
                — ✷ {slide.subtitle} ✷ —
              </p>
              ...
              >
                ...
                <span className="ml-3 sm:ml-4 md:ml-5 flex items-center justify-center h-7 w-7 sm:h-8 sm:w-8 md:h-10 md:w-10 rounded-full bg-black group-hover:bg-[#cae28e] transition-all duration-300 relative z-10">
                  <svg
