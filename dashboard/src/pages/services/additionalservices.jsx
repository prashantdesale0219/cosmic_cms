...
const AdditionalServices = () => {
  ...
  const fetchAdditionalServices = async () => {
    try {
      ...
      setAdditionalServices(response.data.data.additionalServices);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        const response = await api.patch(`/services/additional-services/${editingItem._id}`, formData);
        setAdditionalServices(additionalServices.map(item => 
          item._id === editingItem._id ? response.data.data.additionalService : item
        ));
        toast.success('Additional service updated successfully!');
      } else {
        const response = await api.post('/services/additional-services', formData);
        setAdditionalServices([...additionalServices, response.data.data.additionalService]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/services/additional-services/${id}`);
      setAdditionalServices(additionalServices.filter(item => item._id !== id));
  ...
  if (loading && additionalServices.length === 0) {
    return (
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage the additional services displayed on the services page</p>
        </div>
        <button
          onClick={() => {
            resetForm();
      ...
      {additionalServices.length > 0 ? (
        ...
        <div className="text-center py-12 bg-white rounded-lg shadow-md">
          <p className="text-gray-500 mb-4">No additional services found</p>
          <button
            onClick={() => {
              resetForm();
              setIsModalOpen(true);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {editingItem ? 'Edit Additional Service' : 'Add Additional Service'}
            </h2>
