...
const TeamMembersList = () => {
  ...
  const fetchTeamMembers = async (page = 1, query = searchQuery, sort = sortField, direction = sortDirection) => {
    try {
      setIsLoading(true);
      setError(null);
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    setCurrentPage(1);
    fetchTeamMembers(1, searchQuery);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      console.log('Deleting team member with ID:', memberToDelete._id);
      
      const response = await teamService.deleteTeamMember(memberToDelete._id);
      
      console.log('Delete team member response:', response);
      
      if (response && (response.success || (response.data && response.data.success))) {
        toast.success(response.message || response.data?.message || 'Team member deleted successfully');
        fetchTeamMembers(currentPage);
      } else {
        console.error('Failed to delete team member:', response);
        toast.error(response?.message || response.data?.message || 'Failed to delete team member');
      }
    } catch (err) {
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">{teamMembers.length > 0 ? (currentPage - 1) * 10 + 1 : 0}</span> to{' '}
                      <span className="font-medium">
                        {Math.min(currentPage * 10, (totalPages - 1) * 10 + teamMembers.length)}
                      </span>{' '}
                      of <span className="font-medium">{(totalPages - 1) * 10 + teamMembers.length}</span> results
                    </p>
                  </div>
                  <div>
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Are you sure you want to delete the team member "{memberToDelete?.name}"? This action cannot be undone.
                    </p>
                  </div>
                </div>
              </div>
