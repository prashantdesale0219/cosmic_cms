...
const CategoriesList = () => {
  ...
  const fetchCategories = async (page = 1, query = searchQuery, sort = sortField, direction = sortDirection) => {
    try {
      setIsLoading(true);
      setError(null);
      
      console.log('Fetching categories with params:', { page, limit: 10, search: query, sort, direction });
      
      const response = await categoryService.getCategories(page, 10, query, sort, direction);
      ...
      if (response && response.success) {
        ...
        if (response.data && Array.isArray(response.data)) {
          // If response.data is directly an array of categories
        ...
        console.log('Processed categories data:', { categoriesData, totalPagesData, currentPageData });
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">{categories.length > 0 ? (currentPage - 1) * 10 + 1 : 0}</span> to{' '}
                      <span className="font-medium">
                        {Math.min(currentPage * 10, (totalPages - 1) * 10 + categories.length)}
                      </span>{' '}
                      of <span className="font-medium">{(totalPages - 1) * 10 + categories.length}</span> results
                    </p>
                  </div>
                  <div>
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      ...
                      All content associated with this category may be affected.
                    </p>
                  </div>
                </div>
              </div>
