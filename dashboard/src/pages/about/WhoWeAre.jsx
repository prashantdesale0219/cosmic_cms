  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Who We Are Section</h1>
          <p className="text-gray-600">Manage the who we are content</p>
        </div>
        <button
          onClick={() => {
            resetForm();
        ...
        >
          ...
          {whoWeAre ? 'Edit Who We Are' : 'Create Who We Are'}
        </button>
      </div>
      ...
      {whoWeAre ? (
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">{whoWeAre.title}</h3>
              <p className="text-sm text-gray-500">Status: {whoWeAre.isActive ? 'Active' : 'Inactive'}</p>
            </div>
            <button
              onClick={() => handleEdit(whoWeAre)}
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-gray-900 whitespace-pre-wrap">{whoWeAre.description}</p>
            </div>
          </div>
        </div>
      ) : (
        <div className="text-center py-12">
          <p className="text-gray-500 mb-4">No who we are section found</p>
          <button
            onClick={() => {
              resetForm();
              setIsModalOpen(true);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {editingItem ? 'Edit Who We Are Section' : 'Create Who We Are Section'}
            </h2>
