...
import { FaLeaf, FaSolarPanel, FaHandshake, FaLightbulb, FaUsers, FaHeart, FaLock, FaRocket } from 'react-icons/fa';
import { companyCultureService } from '../services/api';

const CompanyCulture = () => {
  const [heroData, setHeroData] = useState(null);
  const [brandVisionData, setBrandVisionData] = useState(null);
  const [coreValues, setCoreValues] = useState([]);
  const [workEnvironmentData, setWorkEnvironmentData] = useState(null);
  const [sustainabilityCards, setSustainabilityCards] = useState([]);
  const [sustainabilityCommitments, setSustainabilityCommitments] = useState([]);
  const [joinTeamCTA, setJoinTeamCTA] = useState(null);
  ...
  const iconMap = {
    ...
    FaShield: <FaLock className="h-10 w-10 text-[#9fc22f]" />,
  ...
  const fetchCompanyCultureData = async () => {
    try {
      ...
      // Fetch all company culture data
      const [heroResponse, brandVisionResponse, coreValuesResponse, workEnvironmentResponse, sustainabilityCardsResponse, sustainabilityCommitmentsResponse, joinTeamCTAResponse] = await Promise.all([
        companyCultureService.getCompanyCultureHero().catch(() => ({ data: { success: false } })),
        companyCultureService.getBrandVision().catch(() => ({ data: { success: false } })),
        companyCultureService.getCoreValues().catch(() => ({ data: { success: false } })),
        companyCultureService.getWorkEnvironment().catch(() => ({ data: { success: false } })),
        companyCultureService.getSustainabilityCards().catch(() => ({ data: { success: false } })),
        companyCultureService.getSustainabilityCommitments().catch(() => ({ data: { success: false } })),
        companyCultureService.getJoinTeamCTA().catch(() => ({ data: { success: false } }))
      ]);
      ...
      if (brandVisionResponse.data.success) {
        setBrandVisionData(brandVisionResponse.data.data);
      }
      ...
      if (workEnvironmentResponse.data.success) {
        setWorkEnvironmentData(workEnvironmentResponse.data.data);
      }
      
      if (sustainabilityCardsResponse.data.success) {
        setSustainabilityCards(sustainabilityCardsResponse.data.data || []);
      }
      
      if (sustainabilityCommitmentsResponse.data.success) {
        setSustainabilityCommitments(sustainabilityCommitmentsResponse.data.data || []);
      }
      
      if (joinTeamCTAResponse.data.success) {
        setJoinTeamCTA(joinTeamCTAResponse.data.data);
      }
  ...
  return (
    <div className="min-h-screen font-['Space_Grotesk']">
      ...
      <motion.header
        ...
        style={{
          backgroundImage:
            `url('${heroData?.backgroundImage || '/companyculture.jpeg'}')`,
        }}
      >
        <div className="absolute inset-0 bg-black/50" />
        <div className="relative z-10 text-center text-white px-4">
          <h1 className="text-4xl sm:text-5xl font-bold mb-4">{heroData?.title || 'Company Culture'}</h1>
          <nav className="flex items-center justify-center space-x-2 text-sm">
            ...
            <span>—</span>
            <span className="text-[#9fc22f]">{heroData?.subtitle || 'Company Culture'}</span>
          </nav>
        </div>
      </motion.header>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-[#003e63]">
                {brandVisionData?.title && (
                  <span className="text-[#9fc22f]">{brandVisionData.highlightText || brandVisionData.title}</span>
                )} 
                {!brandVisionData?.title && (
              ...
              {brandVisionData?.description1 && (
                <p className="text-gray-700 mb-6">
                  {brandVisionData.description1}
                </p>
              )}
              
              {brandVisionData?.description2 && (
                <p className="text-gray-700 mb-6">
                  {brandVisionData.description2}
                </p>
              )}
              
              {brandVisionData?.description3 && (
                <p className="text-gray-700 mb-6">
                  {brandVisionData.description3}
                </p>
              )}
              ...
              <Link 
                to={brandVisionData?.ctaLink || "/contact"} 
              ...
              >
                <span className="relative z-10 transition-colors duration-300 group-hover:text-white">{brandVisionData?.ctaText || "Join Our Mission"}</span>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="order-2 md:order-1">
              <img 
                src={workEnvironmentData?.image || "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=800&q=80"} 
                alt={workEnvironmentData?.imageAlt || "Collaborative work environment"} 
                className="rounded-lg shadow-lg w-full h-auto"
              />
            </div>
            ...
            <div className="order-1 md:order-2">
              <h2 className="text-3xl font-bold mb-6 text-[#003e63]">
                {workEnvironmentData?.title ? (
                  workEnvironmentData.highlightText ? (
                    <>
                      {workEnvironmentData.title.replace(workEnvironmentData.highlightText, '').trim()}{' '}
                      <span className="text-[#9fc22f]">{workEnvironmentData.highlightText}</span>
                    </>
                  ) : (
                    workEnvironmentData.title
                  )
              ...
              {workEnvironmentData?.description1 && (
                <p className="text-gray-700 mb-4">
                  {workEnvironmentData.description1}
                </p>
              )}
              
              {workEnvironmentData?.description2 && (
                <p className="text-gray-700 mb-4">
                  {workEnvironmentData.description2}
                </p>
              )}
              
              {workEnvironmentData?.description3 && (
                <p className="text-gray-700 mb-4">
                  {workEnvironmentData.description3}
                </p>
              )}
      ...
      >
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {sustainabilityCards && sustainabilityCards.length > 0 ? (
              sustainabilityCards.map((card, index) => (
                ...
                >
                  <div className="h-48 overflow-hidden">
                    <img 
                      src={card.image || "/enviroment.jpeg"} 
      ...
      >
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-3xl sm:text-4xl font-bold mb-8">
            {sustainabilityCommitments?.title ? (
            sustainabilityCommitments.highlightText ? (
              <>
                {sustainabilityCommitments.title.replace(sustainabilityCommitments.highlightText, '').trim()}{' '}
                <span className="text-[#9fc22f]">{sustainabilityCommitments.highlightText}</span>
              </>
            ) : (
              sustainabilityCommitments.title
            )
          ...
          {sustainabilityCommitments?.description || "Beyond our products, we're committed to sustainable operations in every aspect of our business."}
        </p>
          
          <div className="grid gap-8 md:grid-cols-3">
          {sustainabilityCommitments?.commitments && sustainabilityCommitments.commitments.length > 0 ? (
            sustainabilityCommitments.commitments.map((commitment, index) => (
                <motion.div
                  key={commitment._id || index}
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                  viewport={{ once: true }}
      ...
      >
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-6 text-[#13181f]">
            {joinTeamCTA?.title ? (
            joinTeamCTA.highlightText ? (
              <>
                {joinTeamCTA.title.replace(joinTeamCTA.highlightText, '').trim()}{' '}
                <span className="text-[#9fc22f]">{joinTeamCTA.highlightText}</span>
              </>
            ) : (
              joinTeamCTA.title
            )
          ...
          <p className="text-gray-700 mb-8">
            {joinTeamCTA?.description || "We're always looking for talented individuals who share our passion for renewable energy and sustainability. Explore our current openings and become part of our mission to create a greener future."}
          </p>
          
          <Link 
            to={joinTeamCTA?.ctaLink || "/careers"} 
          ...
          >
            <span className="relative z-10 transition-colors duration-300 group-hover:text-white">{joinTeamCTA?.ctaText || "View Career Opportunities"}</span>
            <span className="absolute inset-0 bg-[#003e63] transform translate-x-full group-hover:translate-x-0 transition-transform duration-500 ease-in-out"></span>
          </Link>
        </div>
      </motion.section>
